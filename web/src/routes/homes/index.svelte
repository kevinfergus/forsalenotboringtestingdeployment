...
<script>
  ...
  onMount(() => {
    filteredListings = listings.filter(
      (listing) => listing.price <= $maxPrice && listing.price >= $minPrice
    );
  });
...
<div class="row mt-3 mb-0">
  ...
  <div class="flex px-3 items-baseline justify-between">
    ...
    <button
      ...
      on:click={toggleFilterBox}>Filters</button>
  </div>
  {#if showFilterBox}
    <div transition:slide use:safariWorkaround class="bg-primary px-2 py-1">
      <select
        class="block mb-2 appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
        bind:value={$minPrice}>
        {#each minPrices as p}
          <option value={p.value}>{p.id}</option>
        {/each}
      </select>
      <select
      ...
      <div>
        <button
          ...
          class="bg-secondary text-white text-sm md:text-md uppercase py-1 px-2 mt-2 border border-secondary rounded">Apply</button>

        <button
          on:click={resetPrice}
          class="bg-secondary text-white text-sm md:text-md uppercase py-1 px-2 mt-2 border border-secondary rounded">Reset</button>
      </div>
    </div>
  {/if}
</div>
