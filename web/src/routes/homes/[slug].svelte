...
<div class="container mx-auto mb-2">
  <div
    ...
    alt={listing.address}
  />
  <div class="mx-auto w-full mt-3 px-3">
    <div class="flex w-full justify-between">
      <div class="md-w-1/2">
        <h1 class="text-xl md:text-3xl font-semibold">{listing.address}</h1>
        <h3 class="text-md md:text-2xl text-gray-darker">
          {listing.city}
          {#if typeof listing.neighborhood !== "undefined"}| {listing.neighborhood}{/if}
        </h3>
      </div>
      <div>
        {#if listing.active}
          <div class="bg-success rounded-lg px-2 py-1 md:px-3 md:py-2">
            <span
              class="tracking-wide font-semibold md:text-right text-sm md:text-md text-black uppercase"
              >Active</span
            >
          </div>
        {:else}
  ...
  <div class="flex w-full py-4 mb-2 justify-start flex-row">
    <div
      class="flex flex-col text-center justify-center w-1/4 md:text-2xl sm:w-1/2"
    >
  ...
  {#each images as galleryImg}
    <div class="my-3 mx-auto">
      <img
        ...
        src="{urlFor(galleryImg)}?auto=format"
        alt="alt"
      />
    </div>
  {/each}
